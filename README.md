...
代码按“数据窗口 -> 风险估计 -> 透明优化 -> 回测归因 -> 稳健性验证”的流程组织。每个月末再平衡时，系统只取 `returns.index < rebalance_date` 的历史窗口，筛选当时可投资且观测充足的 ETF，再估计收益、协方差、趋势状态和风险预算，最后通过优化器生成组合权重。

Global RRP 使用宽松风险平价思想，在风险预算基础上加入收益目标、债券杠杆上限、趋势确认和防御性风险覆盖。它是主要的收益效率展示模型，权重由 `src/risk_parity.py` 与 `src/backtest.py` 中的优化流程生成。

Convex Adaptive Global RRP 将风险预算目标写成可加入约束的凸优化近似：目标函数同时考虑组合方差、与自适应预算的偏离、换手惩罚、预期收益项和可选 CVaR 尾部风险惩罚；约束包括权重非负、权重和为 1、单资产上限、组别约束和换手上限。Improved Convex Adaptive Global RRP 在此基础上强调低换手、CVaR 控制和可实施性。

协方差估计由 `src/covariance_estimators.py` 统一管理，支持样本协方差、Ledoit-Wolf 收缩估计和 EWMA 半衰期估计。估计器默认输出日度协方差；进入优化器时显式年化，并记录 PSD 修复、条件数、fallback 和 point-in-time 诊断。

HRP/HERC 使用层次聚类和递归风险分配作为基准模型；图特征、在线状态识别和资产定价诊断只作为解释和验证层，不直接生成最终权重。
...
The code follows a consistent research pipeline: data windowing, risk estimation, transparent optimization, backtest attribution, and robustness validation. At each month-end rebalance, the system uses only historical observations where `returns.index < rebalance_date`, filters ETFs with sufficient investable history, estimates return and covariance inputs, builds risk-budget signals, and then generates portfolio weights through optimization.

Global RRP applies the relaxed risk-parity idea with return targets, bond leverage limits, trend confirmation, and defensive risk overlays. It is the main return-efficient showcase model, with weights generated by the optimization flow in `src/risk_parity.py` and `src/backtest.py`.

Convex Adaptive Global RRP expresses the risk-budgeting target as a constrained convex approximation. Its objective combines portfolio variance, deviation from the adaptive risk budget, turnover penalty, expected-return reward, and optional CVaR tail-risk penalty. Constraints include long-only weights, full investment, single-asset caps, group bounds, and turnover caps. Improved Convex Adaptive Global RRP builds on this layer with stronger emphasis on low turnover, CVaR control, and implementability.

Covariance estimation is centralized in `src/covariance_estimators.py`, covering sample covariance, Ledoit-Wolf shrinkage, and EWMA halflife estimators. The estimator returns daily covariance by default; optimizer integrations explicitly annualize it and record PSD repair, condition number, fallback, and point-in-time diagnostics.

HRP/HERC use hierarchical clustering and recursive risk allocation as benchmarks. Graph features, online regime detection, and asset-pricing diagnostics are explanatory or validation layers only; they do not directly generate final portfolio weights.
