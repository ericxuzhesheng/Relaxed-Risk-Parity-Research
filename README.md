...
过拟合审计详见 [`docs/OVERFITTING_AUDIT.md`](docs/OVERFITTING_AUDIT.md)。下一阶段验证将围绕 walk-forward selection、nested train/validation/test、CSCV/PBO 诊断、冻结样本外区间、参数稳定性和交易成本压力测试展开。当前 README 中的 Improved Convex Adaptive Global RRP 仍按研究扩展披露，不声称已经完成冻结样本外验证。
...
See [`docs/OVERFITTING_AUDIT.md`](docs/OVERFITTING_AUDIT.md) for the overfitting audit. The next validation layer covers walk-forward selection, nested train/validation/test splits, CSCV/PBO diagnostics, a frozen OOS period, parameter stability, and transaction-cost stress tests. Improved Convex Adaptive Global RRP remains disclosed here as a research extension, not as a completed frozen OOS validation.
