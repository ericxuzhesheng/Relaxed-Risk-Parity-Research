...
关于未来函数、候选参数筛选和研究过拟合风险，见 [`docs/OVERFITTING_AUDIT.md`](docs/OVERFITTING_AUDIT.md)。下一阶段验证将围绕 walk-forward parameter selection、nested train/validation/test、CSCV/PBO diagnostic、冻结样本外区间和敏感性测试展开。当前 README 中的 Improved Convex Adaptive Global RRP 作为受约束研究细化披露，并显式披露候选参数筛选风险。
...
For the look-ahead, candidate-selection, and research-overfitting audit, see [`docs/OVERFITTING_AUDIT.md`](docs/OVERFITTING_AUDIT.md). The next validation layer covers walk-forward parameter selection, nested train/validation/test splits, CSCV/PBO diagnostics, a frozen OOS period, and sensitivity tests. Improved Convex Adaptive Global RRP is reported as a constrained research refinement with explicit candidate-selection risk disclosed.
