...
主要输出位于 [`results/tables`](results/tables) 与 [`results/figures`](results/figures)，其中 [`results/tables/asset_descriptive_statistics.csv`](results/tables/asset_descriptive_statistics.csv) 用于论文第四章资产描述性统计。CSCV/PBO 当前为 reduced candidates / capped combinations 下的 intermediate validation evidence；Frozen OOS 若 2025+ 区间已在开发中被观察过，应按 pseudo-frozen 条件性证据解读。所有回测结果均不代表未来收益保证。
