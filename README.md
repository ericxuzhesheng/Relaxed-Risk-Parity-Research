...
最新一轮论文补强已将第 2 章文献综述更紧密地对接论文设计，补入基于 `report/reference` 核验后的风险平价、HRP/HERC、国内实践与低利率绝对收益路径材料；同时新增了第 5 章权重路径与模型行为解释，以及更平衡的 HRP/HERC benchmark 讨论。
...
Global RRP 是主要的收益效率展示模型。Improved Convex Adaptive Global RRP 在当前重跑样本中保持较稳健的风险收益特征，并将平均月度换手率降至 0.21%，体现了凸约束在低换手、尾部风险控制和稳定配置中的价值。HRP/HERC 仅作为层次化风险配置 benchmark；其表现依赖资产池、样本区间和约束设定，不替代 Global RRP 与 Convex Adaptive RRP 框架。
...
The latest thesis reinforcement pass ties the literature review more tightly to the thesis design, adds a dedicated empirical mechanism / weight-path discussion, and expands the benchmark interpretation around HRP/HERC rather than treating them as weak comparators.
