...
论文初稿位于 [`report/thesis_latex/main.tex`](report/thesis_latex/main.tex)，已按本科实证研究型论文要求扩写，并迁移到西南财经大学本科毕业论文 LaTeX 模板格式。主题为宽松风险平价在全球多资产 ETF 配置中的改进与实证研究。论文显式覆盖绪论、文献综述、理论框架、研究设计、实证结果、稳健性检验、机构配置应用讨论、结论与附录。Global RRP 是主要收益效率展示模型，Improved Convex Adaptive Global RRP 是低换手、CVaR-aware、可实施性改进，Defensive Dynamic RRP 是防御型风险覆盖实验，HRP/HERC 是具有竞争力的 benchmark。
...
资产池包含 30 支境内外 ETF，覆盖八个类别：债券类（短融 ETF、可转债 ETF、国债 ETF、信用债 ETF、银华日利 ETF）、A 股宽基（沪深 300、中证 500、中证 1000、科创 50、红利）、中国科技（半导体、芯片、科创芯片、人工智能、卫星）、中国新能源（光伏、新能源）、中国金融（证券）、港股（恒生、恒生科技）、全球股票（纳指、纳指科技、标普 500、日经 225、道琼斯）和大宗商品（黄金、有色、豆粕、油气、煤炭）。数据区间 2018-01-02 至 2026-05-06，评估起点 2021-01-01，月度再平衡，3bps 单边交易成本。
...
主要输出位于 [`results/tables`](results/tables) 与 [`results/figures`](results/figures)，其中 [`results/tables/asset_descriptive_statistics.csv`](results/tables/asset_descriptive_statistics.csv) 用于论文第四章资产描述性统计，[`results/tables/weight_path_stage_summary.csv`](results/tables/weight_path_stage_summary.csv) 与 [`results/figures/weight_path_asset_group_comparison.png`](results/figures/weight_path_asset_group_comparison.png) 用于论文第五章权重路径解释。CSCV/PBO 默认运行（4 候选、6 块、6 组合）为 intermediate validation evidence；增强版 CSCV/PBO（10 块、12 组合）提供额外诊断覆盖但仍有限。Frozen OOS 若 2025+ 区间已在开发中被观察过，应按 pseudo-frozen 条件性证据解读；回顾性 holdout 切片补充时间切片透明度。延长样本稳健性、CVaR 参数敏感性、模型治理文档（[`docs/MODEL_GOVERNANCE.md`](docs/MODEL_GOVERNANCE.md)）已纳入研究框架。所有回测结果均不代表未来收益保证。
