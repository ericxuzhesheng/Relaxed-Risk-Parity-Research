...
主要输出位于 [`results/tables`](results/tables) 与 [`results/figures`](results/figures)，其中 [`results/tables/asset_descriptive_statistics.csv`](results/tables/asset_descriptive_statistics.csv) 用于论文第四章资产描述性统计。CSCV/PBO 默认运行（4 候选、6 块、6 组合）为 intermediate validation evidence；增强版 CSCV/PBO（10 块、12 组合）提供额外诊断覆盖但仍有限。Frozen OOS 若 2025+ 区间已在开发中被观察过，应按 pseudo-frozen 条件性证据解读；回顾性 holdout 切片补充时间切片透明度。延长样本稳健性、CVaR 参数敏感性、模型治理文档（[`docs/MODEL_GOVERNANCE.md`](docs/MODEL_GOVERNANCE.md)）已纳入研究框架。所有回测结果均不代表未来收益保证。
...
Primary outputs are under [`results/tables`](results/tables) and [`results/figures`](results/figures). [`results/tables/asset_descriptive_statistics.csv`](results/tables/asset_descriptive_statistics.csv) supports the thesis research-design chapter. CSCV/PBO is intermediate evidence because the current run uses reduced candidates and capped combinations. Frozen OOS should be read as pseudo-frozen if the 2025+ period was visible during development. Backtest results do not guarantee future performance.
