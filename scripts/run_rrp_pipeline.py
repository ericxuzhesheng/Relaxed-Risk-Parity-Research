...
def _run_no_turnover_cap_ablation(
    ...
    if not dynamic_no_cap.empty:
        ablation_rows.append(
            _summarize(
                "Dynamic_RRP_No_Turnover_Cap",
                dynamic_no_cap,
                eval_start_date,
                no_turnover_config,
            )
        )
    return apply_public_model_labels(pd.DataFrame(ablation_rows))
...
def _run_full_diagnostics(
    ...
    pbo = simplified_pbo_diagnostic(returns, param_grid, config, selection_metric)
    ...
    public_ablation = _run_no_turnover_cap_ablation(
        returns,
        param_grid,
        models,
        eval_start_date,
...
def _print_skipped_diagnostics(diagnostics_mode: str) -> None:
    if diagnostics_mode == "full":
        print("\nDiagnostics mode: full. Heavy diagnostic outputs were refreshed.")
        return

    if diagnostics_mode == "light":
        print("\nDiagnostics mode: light. Heavy diagnostic outputs were not refreshed in this run:")
    else:
        print("\nDiagnostics mode: none. Diagnostic outputs were not refreshed in this run:")
        print("  - results/tables/parameter_stability.csv")
        print("  - results/tables/afml_diagnostics.csv")

    for path in HEAVY_DIAGNOSTIC_OUTPUTS:
        print(f"  - {path}")
    print("Run with --diagnostics full to refresh these outputs.")
...
def main():
    ...
    parser.add_argument(
        ...
        default="light",
        help=(
            "Diagnostic depth. light refreshes fast Dynamic RRP diagnostics; "
            "full also refreshes walk-forward, PBO, and no-turnover-cap ablation."
        ),
    )
    ...
    with _stage("summary and core figures"):
        summaries = []
    ...
    if args.mode in ["dynamic", "full"] and "Dynamic_RRP" in models and not models["Dynamic_RRP"].empty:
        ...
        if args.diagnostics in ["light", "full"]:
            with _stage("light diagnostics"):
                _run_light_diagnostics(summary_df, dynamic, param_grid)
        if args.diagnostics == "full":
            with _stage("full diagnostics"):
                _run_full_diagnostics(
                    ...
                    eval_start_date,
                    config,
                    args.train_window_months,
                    args.selection_metric,
                    args.top_k,
                )

    _print_skipped_diagnostics(args.diagnostics)
