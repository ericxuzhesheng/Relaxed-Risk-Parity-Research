...
- Global Relaxed Risk Parity 是解释基准组合。当前 `global_risk` beta 为 33.657，因此其收益应放在广义多资产风险代理下理解，而不是解释为独立 alpha 预测。
...
- Global Relaxed Risk Parity is the reference RRP portfolio. Its current `global_risk` beta is 33.657, so its returns are interpreted against the broad multi-asset proxy rather than as a standalone alpha forecast.
