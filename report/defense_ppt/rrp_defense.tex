  ...
  \begin{columns}[T]
    \begin{column}{0.50\textwidth}
      ...
      \begin{itemize}
        ...
        \item Black-Litterman（1992）：市场均衡先验，仍需主观输入
        \item \alert{共同结论}：收益率预测不确定性高，\textbf{难以持续击败简单基准}
      \end{itemize}
      \vspace{0.4em}
      \textbf{协方差估计进展}
      \begin{itemize}
        ...
        \item 端到端深度学习（Bongiorno 2025）：性能优越但\alert{难纳入硬约束}
      \end{itemize}
    \end{column}
    \begin{column}{0.47\textwidth}
      \textbf{风险平价与宽松风险预算}
      \begin{itemize}
        ...
        \item Gambeta \& Kwon（2020）：\textbf{宽松 RRP} 偏离惩罚的数学性质
      \end{itemize}
      \vspace{0.4em}
      \textbf{层次化方法}
      \begin{itemize}
        ...
        \item \alert{共同局限}：难以直接纳入 CVaR 约束和换手惩罚
      \end{itemize}
    \end{column}
  \end{columns}
\end{frame}
  ...
  \begin{columns}[T]
    ...
    \begin{column}{0.43\textwidth}
      \begin{block}{统计检验的含义}
        \small
        \begin{itemize}
          \item Improved Convex 的 Sharpe 优势\textbf{统计显著}（$p=\improvedVsGlobalPValue$）
          \item 仅降低换手（Convex Adaptive）不产生显著 Sharpe 提升
          \item \textbf{CVaR 约束 + 参数冻结}共同带来了真实的统计差异
          \item Global/Defensive RRP 对等权的优势\textbf{统计不显著}
        \end{itemize}
      \end{block}
    \end{column}
  \end{columns}
