...
% 等权 / 60-40
\newcommand{\equalNetReturn}{10.65\%}
\newcommand{\equalSharpe}{0.788}
\newcommand{\equalSortino}{1.253}
\newcommand{\equalMaxDD}{-13.90\%}
...
\begin{frame}{PBO 热图：概率回测过拟合}
  ...
  \vspace{-0.2em}
  \footnotesize\centering
  PBO（Probability of Backtest Overfitting）热图展示不同超参数组合的过拟合概率；冻结样本外候选为 candidate\_23，本次完整样本主结果候选为 candidate\_03，二者角色需区分
\end{frame}
...
\begin{frame}[shrink=3]{状态条件协方差：Regime Parity 思想的样本外验证}
  \begin{columns}[T]
    \begin{column}{0.45\textwidth}
      ...
      \vspace{0.3em}
      \begin{block}{盲选结果}
        \scriptsize 设计窗独立选出激进先验：压力态权重 $1/3 \to 2/3$
      \end{block}
    \end{column}
    \begin{column}{0.52\textwidth}
      \scriptsize\renewcommand{\arraystretch}{1.25}
      \textbf{留出窗 2024-01-02 — 2026-06-30（先验未见此区间）}
      \begin{center}
      \begin{tabular}{@{}lccc@{}}
        \toprule
        配置 & Sharpe & 最大回撤 & 净年化 \\
        ...
        \rowcolor{swufeLight}
        \textbf{状态条件} & \textbf{1.350} & $-3.87\%$ & \textbf{5.74\%} \\
        \bottomrule
      \end{tabular}
      \end{center}
      \vspace{0.2em}
      ...
      \begin{alertblock}{诚实边界}
        \scriptsize 改善方向一致但幅度有限；最短的 2025+ 留出窗内 Sharpe 优势消失（回撤/CVaR 仍不劣）。故定位为\textbf{主模型的稳健性补充，而非替换基准}。
      \end{alertblock}
    \end{column}
  \end{columns}
\end{frame}
