...
\newcommand{\equalSharpe}{0.789}
\newcommand{\equalSortino}{1.255}
...
\begin{frame}[shrink=4]{四模型定位与参数冻结策略}
  ...
  \begin{tabular}{@{}l>{\raggedright\arraybackslash}p{6.2cm}>{\raggedright\arraybackslash}p{2.8cm}@{}}
    ...
    \textbf{Improved Convex} & \textbf{+ CVaR 约束 + 换手惩罚（主结果 candidate\_25）} & \textbf{主模型} \\
...
\begin{frame}{PBO 热图：概率回测过拟合}
  ...
  \vspace{-0.2em}
  \footnotesize\centering
  PBO（Probability of Backtest Overfitting）热图展示不同超参数组合的过拟合概率；冻结样本外候选为 candidate\_23，本次完整样本主结果候选为 candidate\_25，二者角色需区分
\end{frame}
...
\begin{frame}[shrink=3]{状态条件协方差：Regime Parity 思想的样本外验证}
  \begin{columns}[T]
    ...
    \begin{column}{0.52\textwidth}
      \scriptsize\renewcommand{\arraystretch}{1.25}
      \textbf{留出窗 2024-01 — 2026-06（先验未见此区间）}
      \begin{center}
      \begin{tabular}{@{}lccc@{}}
        \toprule
        配置 & Sharpe & 最大回撤 & 净年化 \\
...
\begin{frame}{近期净值回撤的答辩口径}
  ...
  \begin{columns}[T,onlytextwidth]
    \begin{column}{0.32\textwidth}
      \begin{block}{现象}
        \footnotesize
        2026-05 与 2026-06 策略月度收益分别为 $-0.29\%$、$-1.31\%$，同期沪深300ETF为正收益。
      \end{block}
    \end{column}
