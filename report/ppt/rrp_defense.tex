...
% 等权 / 60-40
\newcommand{\equalNetReturn}{10.00\%}
\newcommand{\equalSharpe}{0.735}
\newcommand{\equalSortino}{1.179}
\newcommand{\equalMaxDD}{-13.79\%}
\newcommand{\equalMonthlyTurnover}{1.21\%}
...
\begin{frame}[shrink=4]{四模型定位与参数冻结策略}
  ...
  \begin{tabular}{@{}l>{\raggedright\arraybackslash}p{6.2cm}>{\raggedright\arraybackslash}p{2.8cm}@{}}
    ...
    \textbf{Improved Convex} & \textbf{+ CVaR 约束 + 换手惩罚（主结果 candidate\_03）} & \textbf{主模型} \\
  ...
  \begin{columns}[T]
    \begin{column}{0.52\textwidth}
      \textbf{参数固定与验证角色}
      \begin{enumerate}
        \item 历史评价从 36 个候选中选出 candidate\_03
        \item 稳健性与敏感性诊断均固定该候选，不再重选
        \item 伪冻结窗口为 2025-01-02—\evalEndDate{}
        \item 因该区间可能曾被观察，不作为严格未见样本证据
      \end{enumerate}
    \end{column}
...
\begin{frame}{绩效汇总：\evalStartDate{}—\evalEndDate{}，\evalMonths{} 个月度观察点}
  ...
  \begin{center}
    \begin{tabular}{@{}lccccc@{}}
      ...
      \midrule
      等权基准        & \equalNetReturn   & \equalSharpe   & \equalSortino   & \equalMaxDD   & \equalMonthlyTurnover \\
...
\begin{frame}{参数扰动敏感性分析}
  ...
  \begin{columns}[T]
    \begin{column}{0.50\textwidth}
      \footnotesize
      \begin{itemize}
        \item 已测试范围内 Sharpe 对收益倾斜项最敏感；将其设为 0 后 Sharpe 仍为 1.331
      \end{itemize}
    \end{column}
    \begin{column}{0.50\textwidth}
      \footnotesize
...
\begin{frame}{调仓频率敏感性：月度当前排名第一}
  ...
  \begin{alertblock}{解释口径}
    \scriptsize 固定 candidate\_03 参数，仅改变调仓日历。月度净收益与 Sharpe 在本次样本中最高；周度和双周换手升至 \freqWeeklyMonthlyTurnover{} / \freqBiweeklyMonthlyTurnover{}，季度换手最低。月度是预设实施规则，并非根据本次排名事后重选。
  \end{alertblock}
\end{frame}
...
\begin{frame}{PBO 热图：概率回测过拟合}
  ...
  \vspace{-0.2em}
  \footnotesize\centering
  增强版 CSCV 使用 36 个候选、10 个时间区块和 12 个组合分割，PBO=0.250；candidate\_03 同时用于主结果和伪冻结诊断，后者不构成严格未见样本证据
\end{frame}
...
\begin{frame}[shrink=3]{状态条件协方差：Regime Parity 思想的样本外验证}
  \begin{columns}[T]
    \begin{column}{0.45\textwidth}
      ...
      \begin{itemize}
        ...
        \item 防过拟合：先验仅在\textbf{设计窗 2019–2023 盲选}，留出窗评估
      \end{itemize}
      \vspace{0.3em}
      \begin{block}{盲选结果}
        \scriptsize 设计窗独立选出激进先验：$\pi=0.67$、先验权重 $1.00$
      \end{block}
    \end{column}
    \begin{column}{0.52\textwidth}
      ...
      \begin{tabular}{@{}lccc@{}}
        ...
        配置 & Sharpe & 最大回撤 & 净年化 \\
        \midrule
        EWMA 基准      & 1.546 & $-2.08\%$ & 6.40\% \\
        \rowcolor{swufeLight}
        \textbf{状态条件} & \textbf{1.594} & $-2.58\%$ & \textbf{6.40\%} \\
        \bottomrule
      \end{tabular}
      \end{center}\vspace{-0.5em}
      \textbf{完整评价窗 2019–2026}
      \begin{center}\vspace{-0.35em}
      \begin{tabular}{@{}lccc@{}}
        \toprule
        配置 & Sharpe & 最大回撤 & 月换手 \\
        \midrule
        EWMA 基准      & 1.430 & $-4.03\%$ & 2.09\% \\
        状态条件        & 1.419 & $-3.91\%$ & 2.24\% \\
        \bottomrule
      \end{tabular}
      \end{center}\vspace{-0.5em}
    \end{column}
  ...
  \begin{alertblock}{诚实边界}
    \scriptsize 留出窗 Sharpe/CVaR 改善但回撤加深；全窗回撤/CVaR 略优但 Sharpe/换手略差，因此仅作\textbf{主模型的稳健性补充}。
  \end{alertblock}
\end{frame}
...
\begin{frame}{三条核心结论}
  \begin{swufekeypoints}
    ...
    \swufepoint{结论有明确的适用边界}{03}{所有数字均条件于 \evalStartDate{}—\evalEndDate{} 这一特定样本区间和冻结参数设定；Walk-forward、参数扰动和调仓频率检验确认结论在区间内相对稳定。月度调仓本次恰好排名第一，但实施规则并非按事后收益选择，也不代表对未来绩效的预测}
  \end{swufekeypoints}
\end{frame}
  ...
  \begin{columns}[T,onlytextwidth]
    \begin{column}{0.53\textwidth}
      ...
      \begin{alertblock}{解释边界}
        \footnotesize 该实验扣除 5 bps 成本，仅是补充基准；22 个品种中 Y/OI/ZC 较早停更且价格会前向填充，故不复现桥水真实组合。
      \end{alertblock}
    \end{column}
    \begin{column}{0.45\textwidth}
      \scriptsize
      ...
      \begin{tabular}{@{}lrrrr@{}}
        ...
        场景 & 收益 & 波动 & Sharpe & 回撤 \\
        \midrule
        ETF 基准 & \improvedNetReturn{} & \improvedVolatility{} & \improvedSharpe{} & \improvedMaxDD{} \\
        全天候 1.0x & 3.99\% & 2.10\% & 1.035 & -1.58\% \\
        目标波动 8\% & 9.80\% & 7.70\% & 1.037 & -7.21\% \\
        目标波动 10\% & 10.04\% & 8.08\% & 1.018 & -7.21\% \\
        \bottomrule
      \end{tabular}
      }
      \vspace{0.8em}
