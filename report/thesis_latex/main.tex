...
\IfFileExists{../../results/figures/asset_graph_stress_timeline.pdf}{
  \begin{figure}[H]
    \centering
    \includegraphics[width=0.84\textwidth]{asset_graph_stress_timeline.pdf}
    \caption{资产池跨类别相关结构}
...
\input{generated_next_month_holdings.tex}
完整记录覆盖 444 次调仓和 13,320 条资产观察，每期列示全部 30 只 ETF，保留零权重及微小权重。逐项记录信息截止日、交易前漂移权重、目标权重、权重变化和成本。周度收益区分自然周实际收益与新持仓生效后的持有期收益，后者截至下一调仓日前一交易日。样本末周和末次持有期均为截断观察。可筛选工作簿与对应 CSV 位于项目的 results/tables 目录，逐年周度热力图另存为独立 PDF。上表仅作为期末摘要，完整记录以逐周明细为准。
