...
\input{generated_primary_numbers.tex}
...
\begin{abstract}
Risk budgeting reduces reliance on expected-return forecasts. Global multi-asset ETF allocation still faces changing correlations, non-convex equal-risk-contribution constraints and transaction costs. This thesis examines how explicit implementation constraints affect portfolio weights and historical performance.
...
风险预算关注各资产对组合总风险的贡献，对收益预测的依赖较低，可用于构建可解释的长期配置框架\cite{qian2005risk,maillard2010properties}。扩展到全球 ETF 后，需要处理市场收盘时间差异、汇率暴露、上市时间差异及不同资产的尾部风险。调仓产生的成本应从收益中扣除，并单独报告换手。
...
本文检验风险预算参考、集中度约束和换手惩罚对配置结果的影响。正式比较包含七个模型，Improved Convex Adaptive Global RRP 为主模型，其余为对照。约束消融与频率变化用于解释差异。
...
\subsection{可能的边际贡献}
研究使用逐期可投池和凸优化生成权重。约束消融同时报告权重变化、松弛量和对偶变量，以检验约束是否生效。
...
HRP Benchmark 通过层次聚类和递归二分回避协方差矩阵求逆\cite{lopezdeprado2016building}，HERC Benchmark 在簇间加入风险贡献逻辑\cite{raffinot2018hierarchical}。两者适合作为层次化基准，但难以直接表达 CVaR 上界和换手惩罚。华泰证券的国内实践研究也指出，工具约束、再平衡频率与杠杆可得性会显著影响风险平价落地\cite{htsc2020misunderstanding,htsc2021hrp,htsc2024practice,htsc2025lowrate}。

\subsection{文献评述与本文切入点}

既有研究分别解决了收益估计、风险预算、尾部约束和层次化分配问题，缺少在同一可交易资产池中对约束能力、交易成本和权重结构的统一比较。本文以 30 支 ETF 构建回测环境，将 CVaR、换手惩罚和资产组边界纳入宽松风险平价，并以 Equal Weight、60/40 Benchmark、HRP Benchmark 和 HERC Benchmark 为对照检验其历史适用边界。
...
\subsection{Global RRP}
宽松风险平价保留 $w^{RB}$ 作为参考权重，再允许组合在风险、收益下限和债券敞口之间作有限调整。令 $z$ 为实际风险敞口，债券资产满足 $w_i\leq z_i\leq L_iw_i$，其他资产满足 $z_i=w_i$。本文求解的凸目标为
\[
...
Global RRP 将债券、可转债、中国权益、港股、海外权益和商品放入同一框架。它用于比较风险预算配置与主模型的收益和换手差异。
...
主模型冻结既有参数日历，并统一采用周频及取消两项集中度上限的设置。该规格经历史实验后确定，属于事后选择。

跟踪、换手与组别参数记录在配置文件中。主模型不使用预期收益奖励。
...
\]
主模型未启用 CVaR 惩罚或上限。其风险表现应结合实际权重和换手约束解释。
...
Ledoit-Wolf 收缩估计\cite{ledoit2004well}将样本协方差矩阵向结构化目标 $F_t$ 加权平均得到
\[
\hat{\Sigma}_t = (1-\delta) S_t + \delta F_t
\]
本研究使用线性收缩，目标 $F_t$ 为等方差对角矩阵，收缩强度 $\delta$ 由当期历史样本估计。

指数加权移动平均（EWMA）通过对近期观测赋予更高权重，估计时变协方差
\[
\hat{\Sigma}_t = (1-\lambda)\sum_{s=0}^{T-1}\lambda^s r_{t-s} r_{t-s}^T,\quad \lambda = e^{-\ln 2/\tau}
\]
其中 $\tau$ 为半衰期。主模型取 60 个观察，窗口长 252 个观察。

主模型使用 252 个历史观察的 EWMA 协方差，半衰期为 60。协方差先对称化并进行尺度相关的半正定修正，优化过程不显式计算矩阵逆。Ledoit--Wolf 消融保持窗口不变，收缩强度由当期历史数据估计。

\subsection{HERC Benchmark}
\label{sec:herc_baseline}
HERC Benchmark在层次聚类结构中加入风险贡献思想，使簇内和簇间风险分配更接近风险预算逻辑\cite{raffinot2018hierarchical}。与本文主模型相比，HERC Benchmark 不支持直接写入 CVaR 约束、$\ell_1$ 换手惩罚和资产组权重上下限，其分配逻辑完全由协方差矩阵的聚类结构驱动。

HRP Benchmark 和 HERC Benchmark 通过层次聚类分配风险，作为优化方法的对照。两者的结果需结合低波动资产占比解释。
...
\IfFileExists{../../results/figures/asset_graph_stress_timeline.pdf}{
  \begin{figure}[H]
    \centering
    \includegraphics[width=0.98\textwidth]{asset_graph_stress_timeline.pdf}
    \caption{资产池跨类别相关结构}
    \label{fig:asset_graph}
  \end{figure}
}{}

相关结构图只作为历史描述，不参与权重求解。主模型按调仓日前窗口估计风险，并通过凸目标和剩余约束生成组合。

\subsection{样本构造与预处理}
收益来自复权价格的相邻交易日变化，保留真实极端值。价格只向前填充，上市前不回填。每只 ETF 有 60 个有效历史观察后才可进入组合。数据通过 Tushare 获取，缓存覆盖 \dataFirstDate{} 至 \dataLastDate{}，评价区间为 \evalStartDate{} 至 \evalEndDate{}。晚上市资产只参与其可用历史覆盖的部分。
...
\]
Calmar 比率为净年化收益除以最大回撤绝对值。平均月度换手率衡量组合执行稳定性。95\% 日度 CVaR 衡量历史日度损失超过 95\% 分位后的平均损失，用于描述历史尾部风险。

\subsection{回测流程与无前视设计}
每次优化使用严格早于调仓日的历史窗口。回测沿用目标权重乘当日收益并扣成本的记账约定，随后更新漂移权重。评价期含 \primaryObservations{} 个日收益观察和 \primaryRebalances{} 次周频调仓。实际成交价格仍需单独验证。
...
HRP Benchmark & \hrpNetReturn & \hrpVolatility & \hrpSharpe & \hrpSortino & \hrpMaxDD & \hrpMonthlyTurnover\\
HERC Benchmark & \hercNetReturn & \hercVolatility & \hercSharpe & \hercSortino & \hercMaxDD & \hercMonthlyTurnover\\
...
60/40 Benchmark & \sixtyFortyNetReturn & \sixtyFortyVolatility & \sixtyFortySharpe & \sixtyFortySortino & \sixtyFortyMaxDD & \sixtyFortyMonthlyTurnover\\
\bottomrule
\end{tabular}}
\end{table}
...
\subsection{净值、回撤与换手}
\figmaybe{convex_adaptive_nav_comparison.pdf}{主模型与对照累计净值}{fig:nav}
\figmaybe{convex_adaptive_drawdown_comparison.pdf}{主模型与对照回撤}{fig:drawdown}
现金类 ETF 配置有助于解释主模型的低波动。跨模型比较同时涉及频率和约束差异，单因素影响由消融实验检验。
\figmaybe{convex_adaptive_turnover_comparison.pdf}{按自然月汇总的平均换手}{fig:turnover}
...
\figwide{primary_weights.pdf}{主模型权重路径}{fig:weight_path}
平均现金类 ETF 权重为 \primaryCashMean，最大值为 \primaryCashMax。取消现金上限并不等价于增加杠杆，也不保证风险来源充分分散。集中配置于低波动基金，是低组合波动和零利率夏普的重要解释。
\figmaybe{convex_adaptive_cvar_comparison.pdf}{核心模型历史尾部损失}{fig:cvar}
主表沿用历史尾均值统计以保证核心模型之间一致；约束实验采用分位点质量可拆分的精确经验 CVaR，与凸辅助变量形式一致。两者均以 95\% 为研究口径，未搜索置信水平。

\subsection{每周持仓与收益明细}
完整逐周记录保存在仓库 results/tables 目录下的持仓、权重矩阵与周收益 CSV 中，可核查目标权重、漂移权重、换手及成本。自然周收益覆盖该周实际交易日，不能归因于周末新持仓。样本末周为截断观察。逐周目标权重中有 11 只 ETF 在全部调仓日均小到以百分比保留四位小数时显示为零。资产池覆盖范围与实际持仓分散程度因此需要区分。实施阶段采用平方跟踪误差与换手惩罚，并未要求每只资产持有正权重；零初始权重与换手惩罚可以使新增资产长期保持近零配置。逐资产原因仍需结合当期参考权重和约束进行诊断。
...
\end{table}
无风险利率设为零时，主模型夏普达到 \improvedSharpe；同一组合以中债利率为机会成本时夏普为 \primaryChinaBondSharpe。两种口径下的实际收益相同。取消单资产上限但保留现金组上限时，该约束组合仍限制现金集中，不能将其解释为完全放开现金配置。
...
\subsection{频率敏感性}
频率实验保留主模型参数及冻结候选日历，只改变周、双周、月、季度再平衡规则。集中度约束的影响由前述消融单独检验。
\input{generated_rebalance_frequency_table.tex}
\figmaybe{rebalance_frequency_sensitivity.pdf}{主模型参数下的调仓频率比较}{fig:rebalance_frequency_sensitivity}
...
实现检查验证记账和约束，统计结论还受到事后规格选择的影响。归档过拟合检验未作为当前主模型的有效性证明。后续应冻结配置并使用新增数据。
...
六个对照用于比较配置方法。分析同时考虑绝对收益、波动和换手，并单列中债利率口径，以解释高现金权重的影响。

\subsection{研究边界}
固定成本未覆盖实际价差、市场冲击和容量。模型未处理负债、久期缺口或资本占用，机构应用需要另作检验。部分资产上市较晚，前向填价也不能完整表达停牌期间的可交易性。

后续应固定参数、资产池和评价口径，以新增数据检验稳定性。
