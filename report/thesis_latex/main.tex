...
近年来，文献在这一方向上出现了若干新的探索路径。Herculano 的贝叶斯参数化组合策略（BPPP）通过在策略系数上设置先验分布，绕过了对收益和协方差进行显式建模的需要，将参数不确定性直接纳入权重生成过程，为"估计误差条件下如何生成可解释的最优策略"提供了一条贝叶斯视角的理论路径\cite{herculano2026bayesian}。Hellum、Jensen、Kelly 和 Malamud 的研究则在理论层面揭示了大规模多资产条件下均值-方差框架的深层结构性质，其核心结论指出当资产数量 $N$ 超过样本观测数 $T$ 时，特定条件下均值-方差组合的样本外表现可能随资产池规模增加而改善，这对"高维设定下经典均值-方差框架是否仍具理论依据"的讨论提供了新的视角\cite{hellum2026complex}。这些研究表明，关于估计误差与参数不确定性的理论探讨仍在持续演进，并在不同方向上扩展了均值-方差框架的理论边界。
...
然而，CVaR 相关文献并未声称其能够完全消除未来尾部风险。CVaR 的准确性高度依赖于历史回看窗口的选择、置信水平设定和样本中实际出现的极端事件。当未来出现历史样本期未曾遭遇的极端市场状态时，基于历史数据的 CVaR 估计仍可能低估实际的尾部风险。从另一视角，Uhl、Jurt 和 Walliser 借助自然语言处理与情感分析构建了基于人工智能的尾部事件识别框架，研究表明在负面新闻被正确识别的条件下，尾部事件期间的股票收益呈现出续涨（而非逆转）模式，且该效应在不同市场情绪状态下存在明显差异\cite{uhl2024aitail}。这一研究为尾部风险的情境识别提供了机器学习视角的补充证据，同时也印证了仅依赖历史损失分布估计极端尾部风险的内在局限性。国内学术界对低利率环境下绝对收益投资策略的讨论也指出，风险控制优先、跨资产分散策略和执行约束的重要性，往往超越对额外收益的追求承诺\cite{htsc2025lowrate}。因此，将 CVaR 直接诠释为收益增强的来源或极端风险的完整保险工具，均超出了现有文献的理论支撑范围。
...
随机矩阵理论为这一问题提供了严格的理论依据。Laloux、Cizeau、Bouchaud 和 Potters 的开创性研究证明，金融资产实证相关矩阵的绝大多数特征值落入随机矩阵预测的噪声区间，真正携带信息的特征值仅占极少数\cite{laloux1999noise}。这一结果从理论上揭示了样本协方差矩阵在高维金融数据中包含大量噪声的根本来源，为收缩估计和特征值过滤方法提供了直接的理论支撑，也是本文采用 Ledoit-Wolf 收缩估计而非原始样本协方差作为基准风险估计方法的重要动因之一。
...
此外，近年来生成模型在合成金融时间序列领域的进展也值得关注。Alouadi 等人提出的 SBBTS 框架将 Schrödinger Bridge 与 Bass 鞅原理统一，通过联合校准漂移和随机波动率生成接近真实市场统计特性的合成时间序列，在 Heston 模型实验和标普 500 数据增强中均表现出对历史协方差和波动率结构的有效捕捉\cite{alouadi2026sbbts}。这类生成框架目前仍处于前沿探索阶段，但其为组合策略的压力情景模拟和稳健性验证提供了不依赖历史样本分布假设的新路径，具有潜在的方法论价值。
...
正因为如此，HRP/HERC 不能被写成陪跑模型。国内实践层面的讨论同样指出，层次化风险估计能够在相关资产较多时改善分散化效果，并且有利于把相关结构信息引入配置过程\cite{htsc2021hrp}。但它们也依赖聚类稳定性、样本区间和资产池结构，且难以直接表达 CVaR、换手惩罚和复杂资产组约束。文献给出的启示不是”HRP/HERC 天然弱于风险预算模型”，而是”它们代表了另一条有经验价值但约束表达较弱的配置路径”。在这一方向上，Ciciretti 和 Pallotta 将图论工具引入风险平价框架，提出基于最小生成树的网络风险平价（Network Risk Parity）方法\cite{ciciretti2024network}。该方法通过将资产相关结构建模为图网络而非层次树，克服了传统 HRP 仅能捕捉资产间一对一关系的局限，并从图论角度证明所得组合的集中度低于均值-方差组合。其样本外表现在资产数量较多时尤为突出，为层次化配置方法的进一步演化提供了有代表性的新参照。
