...
图\ref{fig:model_workflow}区分年度参数更新与周度权重求解：年度验证的两个区间均早于生效日，冻结后的系数进入每周第二阶段优化。
\begin{figure}[htbp]
\centering
\input{model_workflow.tex}
\caption{Global RRP 的年度校准与周度决策流程}
...
\input{generated_primary_constraints.tex}
\end{table}
...
图\ref{fig:rrp_convex_structure}概括了可行权重与三项目标之间的关系\cite{boyd2004convex,roncalli2013introduction}。
\begin{figure}[htbp]
\centering
\includegraphics[width=0.82\textwidth]{rrp_convex_structure.pdf}
\caption{Global RRP 的可行域与凸目标结构示意}
\label{fig:rrp_convex_structure}
\end{figure}
