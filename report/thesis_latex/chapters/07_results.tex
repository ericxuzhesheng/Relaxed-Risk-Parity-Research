...
{\setlength{\tabcolsep}{9pt}\resizebox{\textwidth}{!}{\input{generated_global_performance.tex}}}
\end{table}
...
\subsection{换手和成本代价}
Global RRP 月均买卖合计换手为 \globalMonthlyTurnover。图\ref{fig:turnover}以零起点、统一线性尺度展示五模型的月均换手，并直接标注数值。五个模型均按周调仓，月均换手反映同一日历下各配置规则产生的交易量。
...
图\ref{fig:cost_drag}进一步比较约定费用对五模型年化收益的影响。图中数值为毛、净年化收益之差，百分数刻度应按收益率差理解，不是相对于毛收益的损失比例。
